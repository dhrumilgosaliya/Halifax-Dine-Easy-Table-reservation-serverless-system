...
async function fetchDataFromAPI() {
    try {
        ...
        const apiKey = 'MOpPmfqqRZ3HWuf2oVnWE8P3q2mMjI723wntp8lx';
        let postData = {
            restaurant_id: "1"
        }
...
async function updateDataFromAPI(req) {
    try {
        let apiUrl = "https://k7gf4d2487.execute-api.us-east-1.amazonaws.com/dev-update-rd/updateRestaurantDetails"
   ...
   const db = admin.firestore();
    ...
    try {
        if (event && event['interpretations'] && event['interpretations'].length > 0) {
            ...
            if (selectedIntent && selectedIntent.name == "GetOpeningHours") {
                ...
                } else if(selectedIntent.confirmationState && selectedIntent.confirmationState === "None") {
                    console.log("HEEEO")
                    response.sessionState.dialogAction = {
                       "type": "ConfirmIntent"
                    };
                ...
                else
                {
                    ...
                    console.log(selectedIntent.slots.Day.value.interpretedValue, "hereeee")
                    let days = operationDetails.find((element)=>{ 
                        console.log(element, "blaaa")
                        return element.day == selectedIntent.slots.Day.value.interpretedValue})
                ...
                else
                {
                    ...
                    let operationDetails = getOperation.body.restaurant_operation_details;
                    operationDetails.forEach((interpretation) => {
                        if (interpretation.day ===  selectedIntent.slots.DayOfTheWeek.value.interpretedValue) {
                            let numberTime = selectedIntent.slots.UpdatedOpeningTime.value.interpretedValue.replace(/:/g, '')
            ...
            else if (selectedIntent && selectedIntent.name == "EditRestaurantLocation") {
                const response = {
                    "sessionState": {
                        "intent": {
                ...
                else
                {
                    ...
                    let req = {
                        restaurant_id: "1",
                        address: selectedIntent.slots.UpdatedRestaurantLocation.value.interpretedValue
                    }
               ...
               let menu_available = foodMenu.filter((element)=>{ 
                ...
                let resID = "1"
                    const itemQuery = restauntrantReviews.where('restaurant_id', '==', resID);
                    try{
                ...
                {
                  //   console.log("zzz")
                    let cancelReservationRef = db.collection(restaurant_reservations).doc(selectedIntent.slots.ReservationID.value.interpretedValue);
                    try{
               ...
               {
                 let updateReservationRef = db.collection(restaurant_reservations).doc(selectedIntent.slots.ReservationID.value.interpretedValue);
                 
                 try{
                    const combinedTimestamp = new Date(`${selectedIntent.slots.NewReservationDate.value.interpretedValue}T${selectedIntent.slots.NewReservationTime.value.interpretedValue}:00`);
                    console.log(combinedTimestamp);
                    const itemSnapshot = await updateReservationRef.update({end_time: combinedTimestamp});
                    console.log(itemSnapshot, "please come here please")
                    return {
                        sessionState: {
                           dialogAction: {
            ...
            else if (selectedIntent && selectedIntent.name == "ReadRestaurantRating"){
    ...
    return {
        dialogAction: {
            ...
            message: {
                contentType: 'PlainText',
                content: "I'm sorry, I couldn't understand your request. Can you please try again?"
            }
        }
    };
};
